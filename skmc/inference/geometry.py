...
class Geometry:
    ...
    def fit(self, theta, weights=None):
        ...
        if weights is None:
            self.normal_mean = jnp.mean(theta, axis=0)
            self.normal_cov = jnp.cov(theta.T)
        else:
            self.normal_mean = jnp.average(theta, axis=0, weights=weights)
            self.normal_cov = jnp.cov(theta.T, aweights=weights)
